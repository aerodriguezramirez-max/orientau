...
// URL de la Edge Function de Supabase que reemplaza a api/claude_proxy.php
const CLAUDE_FN_URL = `${SUPABASE_URL}/functions/v1/claude-proxy`;

// Llama a la Edge Function con la MISMA forma que antes se llamaba a
// claude_proxy.php (fetch + resp.ok + resp.json()), así el resto del
// código que ya consume esto no necesita cambiar nada más.
async function fetchClaudeProxy(payload, signal){
  const { data: { session } } = await sb.auth.getSession();
  const headers = { 'Content-Type': 'application/json' };
  if(session?.access_token) headers['Authorization'] = `Bearer ${session.access_token}`;
  return fetch(CLAUDE_FN_URL, { method: 'POST', headers, signal, body: JSON.stringify(payload) });
}
...
const TOOLTIP_HINTS = [
  ...
  'La IA analizará todas tus respuestas al final del test.',
      ...
      <div class="test-mode-modal">
        ...
        <div class="test-mode-options">
          <div class="test-mode-card quick" onclick="document.getElementById('testModeModal').remove(); startTest('quick');">
            ...
            <div class="tmc-desc">Selección aleatoria representativa de las 500 preguntas. Análisis IA en minutos.</div>
...
async function analyzeResults(){
  ...
  // ── PASO 4: Llamada a IA, YA con las universidades reales como contexto (55→90%) ──
  await animateTo(55, 58, 300, 'Consultando IA de orientación...', 'step4', ['step1','step2','step3']);

  let aiText = '', profile = '';

  // Top universidades/carreras reales que la IA puede mencionar (nunca inventa otras)
  const topForIA = matchedUnis.slice(0, 6).map(u => {
    const careersTxt = (u.matchedCareers.length ? u.matchedCareers.map(mc=>mc.career) : u.careers.slice(0,2)).join(', ');
    return `- ${u.name} (${u.city}${u.isLocal ? ', tu ciudad/región' : ''}): ${careersTxt} · ICFES desde ${u.icfes.min}`;
  }).join('\n');

  // Arrancar la animación "lenta" mientras espera la IA
  let _iaDone = false;
  const iaProgressPromise = (async () => {
    // Sube de 58 a 85 lentamente mientras la IA trabaja (max 14s)
    const steps = [
      [58, 68, 2000, 'Procesando tus resultados con IA...'],
      [68, 78, 3000, 'La IA está comparando tus opciones...'],
      [78, 84, 4000, 'Casi listo el análisis de IA...'],
      [84, 85, 2000, 'Esperando respuesta de IA...'],
    ];
    for(const [from, to, dur, txt] of steps){
      if(_iaDone) break;
      await animateTo(from, to, dur, txt, 'step4', ['step1','step2','step3']);
    }
  })();
  ...
  try {
    ...
    const resp = await fetchClaudeProxy({
        usuario_id: currentUser?.id || 0,
        demo: !!currentUser?.demo,
        system: `Eres un orientador vocacional experto en universidades colombianas. Un algoritmo YA seleccionó las mejores universidades/carreras reales para este estudiante (con datos verificados de ICFES) — te las doy abajo. Tu trabajo es EXPLICAR y personalizar esas opciones específicas, comparándolas y dando un consejo práctico de admisión. NUNCA menciones ni sugieras ninguna universidad o carrera que no esté en la lista que te doy — si no está en la lista, no existe para efectos de tu respuesta. Responde SOLO en JSON sin backticks ni markdown: {"profile":"Perfil vocacional en 3-4 palabras","analysis":"4-5 oraciones: explica por qué esas opciones específicas encajan con el estudiante, compáralas brevemente, y cierra con un consejo concreto de admisión (qué preparar, a qué puntaje apuntar, etc.)"}`,
        messages: [{ role: 'user', content: `Ciudad del estudiante: ${currentUser.ciudad||'Colombia'}. Área dominante: ${topArea}. Áreas de aptitud (orden): ${sortedAreas.slice(0,3).join(', ')}.\n\nUniversidades/carreras ya seleccionadas por el algoritmo (usa SOLO estas):\n${topForIA}` }]
      }, controller.signal);
  ...
  _iaDone = true;
  await iaProgressPromise; // esperar que termine la animación parcial
  if(!profile) profile = profileMap[topArea] || topArea;
  if(!aiText)  aiText  = `Tu perfil destaca en el área de ${topArea}. Tienes habilidades clave para destacarte en programas universitarios relacionados con esta área en Colombia.`;

  await animateTo(85, 90, 300, 'Análisis de IA listo ✓', 'step4', ['step1','step2','step3']);
...
function downloadResult(score, areas, profile){
  ...
  c.fillText(`orientau.vercel.app  ·  Generado con IA  ·  ${new Date().toLocaleDateString('es-CO')}`, 450, 548);
...
function renderResults(score, areas, profile, aiText, unis, recommendedCareers){
  ...
  document.getElementById('resultDesc').textContent    = `Basado en tus ${QUESTIONS.length || Object.keys(areas).length > 0 ? (QUESTIONS.length || 30) : 30} respuestas analizadas con IA, identificamos tus fortalezas y las mejores universidades colombianas para tu perfil.`;
...
async function expAnalyze(){
  ...
  try {
    ...
    const resp = await fetchClaudeProxy({
        usuario_id: currentUser?.id || 0,
        demo: !!currentUser?.demo,
        system: `Eres un orientador universitario colombiano experto. Responde SOLO en JSON sin markdown: {"summary":"2-3 oraciones sobre el perfil del estudiante y qué busca","top_carrera":"Carrera más recomendada","razon":"Por qué esa carrera en 1 oración","consejo":"Consejo práctico para la admisión"}`,
        messages:[{role:'user',content:`Ciudad: ${EXP_STATE.city||'Colombia'}. Carrera de interés: ${EXP_STATE.career||EXP_STATE.area||'no especificada'}. Habilidades: ${EXP_STATE.skills.join(', ')||'variadas'}. ICFES aprox: ${EXP_STATE.icfes}. Prioridad: ${EXP_STATE.priority||'calidad'}. Top universidades encontradas: ${scored.slice(0,3).map(u=>u.name).join(', ')}.`}]
      }, controller.signal);
    clearTimeout(timeout);
    ...
    aiText = {
      summary: `Tu perfil combina interés en ${EXP_STATE.career||EXP_STATE.area||'múltiples áreas'} con un ICFES de ${EXP_STATE.icfes} puntos. Encontramos universidades compatibles con tu perfil en Colombia.`,
      top_carrera: EXP_STATE.career || 'Tu carrera de interés',
...
const FAQ_DATA = [
  ...
  { cat: 'Privacidad y datos', q: '¿OrientaU usa Inteligencia Artificial? ¿Cómo?', a: 'Sí — al terminar el test, un algoritmo elige primero las universidades y carreras reales que mejor calzan contigo, y luego una IA (Claude, de Anthropic) redacta la explicación personalizada. La IA solo puede hablar de las opciones que el algoritmo ya seleccionó — no puede inventar universidades nuevas.' },
  { cat: 'Privacidad y datos', q: '¿Le mandan mi nombre o mi foto a la Inteligencia Artificial?', a: 'No. Solo se envían tu ciudad y tus resultados del test (sin nombre, correo, foto ni teléfono) — lo mínimo necesario para generar el análisis.' },
...
async function sendChatMessage(){
  ...
  const systemPrompt = `Eres el asistente de orientación vocacional de OrientaU, una plataforma para estudiantes de colegio en Colombia. SOLO respondes preguntas sobre carreras universitarias, universidades colombianas, el proceso de admisión y el ICFES/Saber 11.

Tienes acceso a este catálogo VERIFICADO de universidades (nombre, ciudad, carreras, ICFES de referencia). Es tu ÚNICA fuente de universidades — no existen otras para efectos de esta conversación:

${getChatUniCatalog()}

Reglas estrictas:
1. NUNCA menciones ni inventes una universidad que no esté en el catálogo de arriba. Si preguntan por una que no está ahí, dilo honestamente y sugiere buscar en el sitio oficial de esa universidad.
2. Si preguntan algo fuera de carreras/universidades/admisión, redirige amablemente: eres un asistente de orientación vocacional, no puedes ayudar con eso.
3. Los puntajes ICFES del catálogo son estimaciones de referencia, no cifras oficiales exactas — acláralo si preguntan directo por un puntaje.
4. Tono cálido y cercano, como hablándole a un estudiante de colegio.
5. Respuestas cortas (máximo 3-4 oraciones) — esto es un chat, no un ensayo.
6. Responde en texto plano, sin markdown ni JSON.`;
  ...
  try {
    ...
    const resp = await fetchClaudeProxy({
        ...
        demo: !!currentUser?.demo,
        system: systemPrompt,
        messages: chatHistory.slice(-10) // últimos 10 mensajes, controla costo
      }, controller.signal);
    clearTimeout(timeout);
    ...
    if(!resp.ok || d?.ok === false){
      const reason = d?.msg ? ` (${d.msg})` : '';
      chatHistory.push({ role: 'assistant', content: `No pude conectarme con la IA en este momento${reason}.`, error: true });
    } else {
      const txt = (d?.content?.map(c => c.text||'').join('') || '').trim();
      chatHistory.push({ role: 'assistant', content: txt || 'No entendí tu pregunta, ¿la puedes reformular?' });
    }
  } catch(e){
    document.getElementById('chatTyping')?.remove();
    chatHistory.push({ role: 'assistant', content: 'Se agotó el tiempo de espera. Intenta de nuevo.', error: true });
  }
...
async function getQuickRecommendation(){
  ...
  // base completa y se le pide a la IA elegir algo razonable a nivel
  // nacional, dejándolo claro en la respuesta.
  let pool = UNIVERSITIES.filter(u => isLocalUni(u.city, u.region || '', city));
  let isNational = false;
  if(pool.length === 0){ pool = UNIVERSITIES; isNational = true; }

  const catalog = pool.map(u => `${u.name} (${u.city}): ${u.careers.join(', ')} — ICFES desde ${u.icfes.min}`).join('\n');

  const systemPrompt = `Eres un orientador vocacional colombiano. Tu única tarea es recomendar UNA sola universidad de la lista de abajo para un estudiante que vive en "${city}". No conoces su carrera de interés todavía, así que elige la opción más sólida y versátil de la lista para alguien que aún no decide qué estudiar.

Lista de universidades reales disponibles (tu ÚNICA fuente, no existen otras para esta respuesta):
${catalog}

Responde SOLO en JSON sin backticks ni markdown:
{"universidad":"nombre EXACTO tal cual aparece en la lista","motivo":"2-3 oraciones explicando por qué es una buena opción general, mencionando que para una recomendación 100% personalizada lo ideal es hacer el test vocacional completo"}`;
  ...
  try {
    ...
    const resp = await fetchClaudeProxy({
        ...
        demo: !!currentUser?.demo,
        system: systemPrompt,
        messages: [{ role: 'user', content: `Recomiéndame 1 universidad para alguien en ${city}.` }]
      }, controller.signal);
    clearTimeout(timeout);
    ...
    if(!resp.ok || d?.ok === false){
      const reason = d?.msg ? ` (${d.msg})` : '';
      resultEl.innerHTML = `<div class="chat-msg assistant error">No pude generar la recomendación${reason}. Intenta de nuevo.</div>`;
    } else {
      const txt = (d?.content?.map(c => c.text||'').join('') || '').replace(/```json|```/g,'').trim();
      let parsed = null;
      try { parsed = JSON.parse(txt); } catch(_) {}

      if(parsed?.universidad){
        const uni = UNIVERSITIES.find(u => u.name === parsed.universidad);
        resultEl.innerHTML = `
          <div style="background:rgba(79,142,247,0.08);border:1px solid rgba(79,142,247,0.25);border-radius:var(--radius-sm);padding:16px;">
            ...
            <div style="font-size:1.05rem;font-weight:700;margin-bottom:4px;">${escapeHtml(parsed.universidad)}</div>
            ${uni ? `<div style="font-size:0.78rem;color:rgba(255,255,255,0.5);margin-bottom:10px;">📍 ${escapeHtml(uni.city)} · ICFES desde ${uni.icfes.min}</div>` : ''}
            <div style="font-size:0.85rem;line-height:1.6;color:rgba(255,255,255,0.85);">${escapeHtml(parsed.motivo || '')}</div>
            ${uni ? `<a href="${uni.url}" target="_blank" style="display:inline-block;margin-top:12px;font-size:0.82rem;color:var(--blue-300);">🌐 Visitar sitio oficial →</a>` : ''}
          </div>`;
      } else {
        resultEl.innerHTML = `<div class="chat-msg assistant error">No pude entender la respuesta. Intenta de nuevo.</div>`;
      }
    }
    ...
    const resultEl = document.getElementById('quickRecResult');
    resultEl.innerHTML = `<div class="chat-msg assistant error">Se agotó el tiempo de espera. Intenta de nuevo.</div>`;
    document.getElementById('quickRecForm').style.display = 'none';
    resultEl.style.display = 'block';
  }
}
